...
class TaskRemoteDataSource {
  ...
  Future<List<Task>> getMyTasks(
    ...
    final response =
        await _apiClient.get('/task-distribution/my-tasks', params: params);
  ...
  Future<TaskDetail> getTaskDetail(String taskId) async {
    final response =
        await _apiClient.get('/task-distribution/assigned-tasks/$taskId');
    print('Response from getTaskDetail: ${response.data}');
    return TaskDetail.fromJson(response.data['data']);
  }

  Future<void> submitAudioTask(String taskId, int batch, bool isTest,
  ...
  Future<void> submitTextTask(String taskId, int batch, bool isTest,
      Map<String, String> textOutputs) async {
    await _apiClient.post(
      '/task-distribution/$taskId/contribute',
      data: {
        'batch': batch,
        'is_test': isTest,
        'attempts': [
  ...
  Future<List<dynamic>> getSubmissionHistory(String microTaskId) async {
    final response = await _apiClient.get(
        '/task-distribution/contributor-micro-task-submissions/$microTaskId');
